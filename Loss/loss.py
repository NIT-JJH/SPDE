...
from Model.Popnet.sobel import Sobel
...
def similarity_loss(sal_depth, nosal_depth):
    sal_grad = get_gradient(sal_depth)
    sal_grad_x, sal_grad_y = sal_grad[:, 0], sal_grad[:, 1]
    sal_ones = torch.ones_like(sal_grad_x)
    depth_normal = torch.stack((sal_grad_x, sal_grad_y, sal_ones), dim=1) # 修改此处

    nosal_grad = get_gradient(nosal_depth)
    nosal_grad_x, nosal_grad_y = nosal_grad[:, 0], nosal_grad[:, 1]
    nosal_ones = torch.ones_like(nosal_grad_x)
    nosal_depth_normal = torch.stack((nosal_grad_x, nosal_grad_y, nosal_ones), dim=1) # 修改此处

    similarity = cos(depth_normal, nosal_depth_normal) # 计算余弦相似度
    # loss = torch.mean(similarity) # 损失为相似度的均值
    # loss = torch.mean(similarity.abs())
